...
| 版本 | 日期 | 主要变更 |
|---|---|---|
| **v0.3.69** | 2026-05-12 | 抖音 discovery 新增首页推荐流 `feed`；候选池新增 `[scheduler.pool_source_shares]`，默认 B 站 / 小红书 / 抖音 = 8 / 1 / 1；YouTube 初始化画像接入 `init --yes-youtube`、`fetch-youtube` 和 `import-youtube`；小红书 bootstrap 默认复用 6 小时内任务并领取后标记 `in_progress`，避免重复打开前台 tab 拉收藏 / 点赞 |
