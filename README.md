...
## 🍴 本 fork 在 upstream 基础上的额外功能

这个 fork 在 [whiteguo233/OpenBiliClaw](https://github.com/whiteguo233/OpenBiliClaw) 上做了几处独立扩展，目标是把"跨平台推荐"这件事在更多 surface 上跑通：

- **🍎 Safari (macOS) 端原生打包** —— 复用 Chrome / Firefox 的同一份 MV3 代码，通过 XcodeGen 生成 Safari Web Extension 宿主 app，本地 ad-hoc 签名即可在 Safari 中加载使用。
- **⭐ 稍后再看** —— 推荐卡和惊喜推荐卡都加了星标按钮，本地持久化在 SQLite，跨 popup / 移动端 Web / 桌面端 Web 三个 surface 状态同步。
- **🔁 手动标记搬运** —— 推荐卡新增「标记为搬运」按钮，bypass `is_likely_repost` 启发式，直接搜 YouTube 原版并把这条改向；对那些标题完全中文、AI 配音类的隐性搬运视频有用。
- **🚫 平台占比严格执行** —— `pool_source_shares` 接受 `share=0` 作为「主动排除该平台」的显式信号，pool cap 会真的把存量条目 trim 掉，而不只是停止新发现。
- **🧠 评论辅助搬运识别**（可选）—— 启用 `sources.youtube.use_comments_for_detection` 后，对标题 / 描述都没有信号的「borderline」候选会抓取 top 评论，让 commenters 的"原视频在 YouTube"线索参与判定，单次响应内并发请求并设上限避免烧光 cookie 配额。
