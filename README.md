...
**多样性选择**——通过评估的候选再经过 平台配额预留 → 主题去重 → 风格均衡 → **跨平台混排** → 数量封顶，避免“一刷都是 AI”。各平台保存配比默认 B 站 / 小红书 / 抖音 / YouTube = 8 / 1 / 1 / 1，可在 `[scheduler.pool_source_shares]` 调整；开箱只启用 B 站，其余平台需显式打开。
...
│   └── storage/               # 数据存储层
├── extension/                 # Chrome 浏览器插件 (B 站 + 小红书 + 抖音 + YouTube + 自启动/配置修复)
├── skills/                    # 内置 Skill 定义
├── docs/                      # 项目文档
└── tests/                     # 测试 (1900+)
```
