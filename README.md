...
- 🔍 **多源发现策略** — B 站四策略（搜索 · 关联链 · 趋势 · 跨域探索，搜索冷却时可由扩展打开真实搜索页抓渲染结果兜底）+ 小红书三层安全发现 + 抖音 DOM-first search / hot / feed + X 搜索 / For-You / 关注作者，跨平台协同工作
...
| 来源 | 发现方式 | 说明 |
|------|----------|------|
| **B 站** | Search · Trending · Related Chain · Explore · 扩展搜索任务桥 | 四大策略均衡协作，API 直连为主；服务端 search 冷却且扩展在线时，后端可入队浏览器搜索任务，扩展打开真实搜索页并回传渲染结果 |
| **小红书** | 被动收集 · 关键词搜索 · 创作者订阅 · 初始化画像导入 | 扩展驱动，安全无风控 |
| **抖音** | 初始化画像导入 · 后台搜索 · 热点关联 · 首页推荐流 | 扩展驱动，强信号入画像；日常发现从抖音首页开始模拟 DOM 操作触发加载，并被动收集页面响应 / 渲染结果，不抢用户焦点 |
...
**安全取数**——B 站和通用 Web 由后端直连（B 站走 WBI 签名 API），但当 B 站 search 被风控冷却且扩展在线时，后端已有任务桥可入队搜索任务，扩展会在后台打开真实登录浏览器的搜索页并抓取已渲染的 DOM 结果作为兜底候选。小红书 / 抖音 / YouTube 则由浏览器扩展在你*已登录的页面*里读取数据：初始化画像默认不深滚、分批回传，后端既不直接爬取也不代登录，YouTube 还可用 Google Takeout 离线导入旧历史。X 则由后端用扩展同步来的 x.com cookie（`auth_token`+`ct0`）做服务端只读重放，扩展只负责同步 cookie 和捕获你自己的互动。日常补池时，抖音 search / hot / feed 都在后台 tab 先打开抖音首页，再用真实 DOM 操作触发搜索、热点或推荐流加载，插件只被动监听页面自己发出的响应和已渲染 DOM；YouTube 由后端按平台缺口直接补。
...
| 抖音交互 | 扩展 DOM + MAIN-world 被动 fetch tap + 插件任务调度；初始化导入发布 / 收藏 / 点赞 / 关注信号，search / hot / feed discovery 从抖音首页模拟 DOM 操作触发加载，并被动收集页面响应 / 渲染结果（零后端爬取） |
...
最新版本：**v0.3.128 / extension v0.3.83: 抖音 DOM-first discovery（2026-06-18）**。最近更新见上方摘要；完整历史见 [docs/changelog.md](docs/changelog.md)。普通用户从 [Latest Release](https://github.com/whiteguo233/OpenBiliClaw/releases/latest) 的 `openbiliclaw-v*` 聚合页同时下载插件包和桌面安装包；后端源码自动更新仍看 `backend-v*` tag。
