...
> 名字起源于 B 站（`Bili` = Bilibili，`Claw` = 爪子），项目最早只支持 B 站。从 v0.3.0 起已扩展为通用跨平台 Agent —— 已落地 B 站 / 小红书 / 抖音 / YouTube / X / 知乎 / Reddit 初始化信号、抖音 search / hot / feed、X 服务端内容发现、知乎插件 search / hot / feed / creator / related discovery、Reddit 插件 search / hot / subreddit / related discovery 和通用 Web 多类源，持续接入更多内容平台。
...
| **Reddit** | 在同一浏览器打开 https://www.reddit.com 正常登录 | `fetch-reddit`、`discover --source reddit` 和 `discover-reddit*` 拉不到数据 |

小红书、抖音、YouTube、知乎和 Reddit 走 Chrome 插件任务链路，X 走服务端 cookie 重放（扩展只负责同步 x.com cookie + 捕获互动），都不需要你额外启动 CDP 调试 Chrome。`[sources.browser].cdp_url` 只保留给通用 Web / 自定义网页源的浏览器抓取场景。
...
- 🌐 **跨平台内容源** — 从 B 站起步，已扩展到小红书、抖音、YouTube、X（Twitter）、知乎和 Reddit 初始化信号 / 抖音 search / hot / feed discovery / X 服务端 cookie 重放发现 / 知乎插件 search / hot / feed / creator / related discovery / Reddit 插件 search / hot / subreddit / related discovery 和通用 Web，架构支持持续接入更多平台。你的兴趣不再被单一平台割裂
...
│   │   ├── reddit_tasks       # Reddit 插件任务队列 / bootstrap/search/hot/subreddit/related
...
| Reddit 交互 | 扩展任务调度在已登录浏览器内读取 bootstrap saved/upvoted/subscribed 与 search / hot / subreddit / related 候选；帖子 / 评论为纯文本卡片 |
...
最新版本：**v0.3.149 / extension v0.3.100 / desktop v0.3.149: Reddit 来源接入（2026-06-30）**。最近更新见上方摘要；完整历史见 [docs/changelog.md](docs/changelog.md)。普通用户从 [Latest Release](https://github.com/whiteguo233/OpenBiliClaw/releases/latest) 的 `openbiliclaw-v*` 聚合页下载插件包和可用桌面安装包；自动化频道 release 仍分别保留 `backend-v*`、`extension-v*`、`desktop-v*`。

## 🗺️ 后续规划

OpenBiliClaw 的目标是做你的**全网个性化内容入口**——从 B 站起步，已落地小红书、抖音、YouTube、X、知乎和 Reddit 初始化信号，抖音 search / hot / feed discovery、X（Twitter）服务端发现、知乎插件 search / hot / feed / creator / related discovery、Reddit 插件 search / hot / subreddit / related discovery 与通用 Web 适配器，下一步：
