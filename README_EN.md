...
> 🇨🇳 **Mainland China downloads (current v0.3.219)**: all 7 latest packages (extension, regular and embedding-enabled macOS / Windows installers) are available from [123 Cloud domestic download](https://4001474255.share.123pan.cn/123pan/IxbZMh-90KO3) via a permanent share; see the [Gitee v0.3.219 release](https://gitee.com/whiteguo233/openbiliclaw/releases/tag/openbiliclaw-v0.3.219) for the smaller attachments and source.
...
2. **Install the backend** — grab the desktop installer from the same [Latest Release](https://github.com/whiteguo233/OpenBiliClaw/releases/latest) (macOS `.dmg` / Windows `.exe`, works out of the box, lives in the menu bar / tray). Each platform ships two variants: the **lean** installer (default; downloads the bge-m3 embedding model on first launch) and the **`-with-embedding`** installer (bge-m3 baked in, ~1.1GB, offline-ready) — pick with-embedding for a poor / offline network, lean otherwise. **Mainland China users can also download all 7 latest packages from [123 Cloud (v0.3.219)](https://4001474255.share.123pan.cn/123pan/IxbZMh-90KO3).** Or, to customize or edit the source, paste this into Claude Code / Codex CLI / Cursor or another AI coding agent:
...
> 🇨🇳 For mainland China downloads, use [123 Cloud (current v0.3.219)](https://4001474255.share.123pan.cn/123pan/IxbZMh-90KO3); the permanent share contains all 7 latest packages. The extension, regular Windows installer, and source are also available from GitHub / the [Gitee v0.3.219 release](https://gitee.com/whiteguo233/openbiliclaw/releases/tag/openbiliclaw-v0.3.219).
