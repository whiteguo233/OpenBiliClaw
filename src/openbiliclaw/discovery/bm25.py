...
CJK text is tokenised as character unigrams + bigrams, Latin text as
lowercased word tokens. Pure-Python, deterministic, no third-party
dependencies.
"""
...
def cjk_tokenize(text: str) -> list[str]:
    """Tokenise ``text`` into CJK unigrams + char bigrams and lowercased Latin words.

    Consecutive CJK run ``abcd`` -> ``a b c d ab bc cd`` (unigrams keep
    single-character interests such as ``车`` matchable, bigrams carry the main
    lexical signal); Latin run -> one word per ``[a-z0-9]+``; punctuation and
