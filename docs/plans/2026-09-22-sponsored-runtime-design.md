...
- Private repo `openbiliclaw-sponsored-runtime`（私有仓 `main` 最新 `76b3745`）：Ed25519 Policy 验签、contract 校验、长度帧 IPC、quota 账本（HMAC key 存 macOS Keychain / Linux secret-tool，0600 file fallback，支持旧常量 key 一次性迁移）、Key fragment 与发布公钥 build-time 内嵌、`wrap-key`/`sign-policy`/`public-key` 工具、真实 SiliconFlow HTTP client、`/user/info` 余额探针 + 分级降级，43 个 Rust 测试通过（另有 1 个 macOS Keychain 测试 `--ignored` 手动验证通过）；
...
- 待完成：release workflow 实跑（protected environment/secrets、多平台打包、产物签名）、Windows DPAPI 账本存储、公开侧 Sponsored pilot task eval 回归。
