...
- Public Phase 1/2 已完成并在 PR #263（最新 `44b1937c`）验证：contracts / policy 生成器 / provider / protocol / mock runtime / typed error fallback / `[llm.sponsored]` 配置 / eval harness；**Sponsored 与 BYOK 发送完全相同的旧 system/user prompt，batch 对齐旧流程 32，max_output 对齐 16384**；runtime 只验 system prompt hash、task 白名单、输入字节上限、model/max_output；成功响应写入项目 `llm_usage`，provider=`sponsored` 成本估算固定 ¥0；
- Private repo `openbiliclaw-sponsored-runtime`（`main` 最新 `2833b26`）：Policy 验签、contract 校验（支持 object schema 或 legacy string payload）、IPC、quota 账本（reserve/commit 修正）、Key fragment/公钥 build-time 内嵌、`wrap-key`/`sign-policy`/`public-key`、真实 HTTP client、余额降级、release matrix、对抗测试与 threat model，52 个 Rust 测试通过（另有 1 个 Keychain 测试 `--ignored`）；
- **免费模型约束**：最终 pilot policy 固定 `THUDM/GLM-4-9B-0414`（SiliconFlow 免费，用户侧 ¥0）。真实干净实例 `profile-consolidate --full --apply`：旧流程 prompt + batch 32，21 个嫌疑簇单批，`merges=20`、`errors=0`、likes 44→25、dislikes 4→3，1 个 banned canonical 被拒，0 fallback；runtime 账本 1 请求 / 3324 tokens，`llm_usage` 记录 `sponsored / THUDM/GLM-4-9B-0414 / 2628+696`；mock BYOK 0 次 chat 调用；第二次 dry-run 只剩 1 个 dislike 合并、无错误；
- 免费候选实测：`DeepSeek-R1-0528-Qwen3-8B` 空闲时最强（8/19/32 簇合法 JSON），但当前服务端对真实 consolidation payload 稳定 400 `20012 Model does not exist`；`Qwen/Qwen3-8B`、`THUDM/GLM-Z1-9B-0414`、`Qwen/Qwen2.5-7B-Instruct` 会串簇；`Qwen/Qwen3.5-4B` 在 ≥4 簇返回 503；`XingChenAGI/Xing4.0-29B` 8/17/22 簇 repetition 或 0 ops（max_tokens 提到 32768 也无效）；因此 GLM-4-9B 是当前唯一可用的免费模型；
- `CONSOLIDATION_MAX_OUTPUT_TOKENS` 直接引用旧流程 `DEFAULT_STRUCTURED_MAX_TOKENS`；max_output 4096 被 thinking 吃光、runtime usage key 不匹配、quota reserve 漏记等真实问题均已修复；
- 待完成：release workflow 实跑（protected environment/secrets、notarize/Authenticode、多平台打包），以及在真实用户数据上继续观察 GLM-4-9B 的误合并率后决定默认启用。
