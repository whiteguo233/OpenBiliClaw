...
- 旧流程 prompt 实测：`XingChenAGI/Xing4.0-29B` 在 8/17/22 簇均出现 repetition 或 0 ops；把 `max_tokens` 提高到 32768 后 19 簇仍是 `finish_reason=stop` / 0 ops / 仅 246 输出 token，说明不是上下文或输出预算不足，而是模型无法遵循旧 consolidation prompt，因此 pilot policy 固定能跑通旧 prompt 的 DeepSeek-V3.2；`CONSOLIDATION_MAX_OUTPUT_TOKENS` 现在直接引用旧流程 `DEFAULT_STRUCTURED_MAX_TOKENS`，prompt/batch/temperature/response_format 也全部复用旧流程；max_output 4096 会被 thinking 吃光导致空响应的问题已随对齐 16384 修掉；runtime usage key 与项目 recorder 已统一；quota reserve 漏记 token 已修；
