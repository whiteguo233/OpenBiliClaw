...
| Reddit 后台 discovery producer | ✅ | `RedditDiscoveryProducer.produce_if_due()` 在 Reddit 平台族低于 quota 且 `[sources.reddit].enabled=true` 时，通过已安装浏览器插件的真实 `reddit.com` 登录态触发 `search` / `hot` / `subreddit` / `related` 四个分支；四个分支各自有独立 daily budget，默认每类 300。producer 只 enqueue raw candidates 到 `discovery_candidates`，不写 `content_cache`、不同步跑 LLM 评估，正式 admission 由共享 evaluator 异步完成。 |
...
Reddit 的 steady-state discovery 默认走浏览器插件任务桥。`produce_if_due()` 在 `[sources.reddit].enabled=true`、Reddit 平台族低于 quota、距上次执行已过 `min_interval_minutes` 时，按 `[sources.reddit].source_modes` 调度四类分支：
...
默认 `backend="extension"`：后端写入 `reddit_tasks`，插件唤醒真实 `reddit.com` 登录态 tab；init 期 `bootstrap_events` 读取 saved / upvoted / subscribed，discovery 期通过同源 `.json` endpoint 读取 posts / comments，再 POST `/api/sources/reddit/task-result` 回写。`opencli` / `rdt` 只保留为 discovery 兼容 fallback。每条内容经 `reddit_items_to_contents()` 映射为 `DiscoveredContent(source_platform="reddit", source_strategy="reddit-<mode>")`，posts / comments 会保留 `body_text` 与 `content_type ∈ {"post", "comment"}`，前端因此按无封面文字卡展示。
