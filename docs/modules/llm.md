...
| Sponsored Runtime 合约（Phase 1/2，未发布） | 🚧 | 新增 `sponsored_contracts.py` / `sponsored_tasks.py` / `prompt_contracts.py` / `sponsored_provider.py` / `sponsored_protocol.py` / `sponsored_mock_runtime.py` / `sponsored_errors.py` 与 `scripts/generate_sponsored_policy.py`：canonical JSON-mode 入口收口为 `ensure_json_mode_contract()`（发送与 policy 生成同源）；pilot contract 为 `soul.consolidation.v1`（Sponsored 与 BYOK 发送完全相同的旧 system/user 消息；runtime 只校验 system prompt hash、task 白名单、输入字节上限、模型与 max_output，batch 对齐旧流程的 32）；Provider 实现长度帧 IPC、握手、超时/取消与 typed error fallback 分类，mock runtime 供测试/社区开发。`LLMService.execute_sponsored_task()` 与 `task_options.execute_sponsored_or_structured()` 已完成路由：默认关闭（`OPENBILICLAW_SPONSORED_ENABLED=false`，未配置即完全走 BYOK/Ollama）；可用性/配额/限流错误回退，`CONTRACT_MISMATCH` 失败不进 fallback。`[llm.sponsored]` 配置（`enabled` / `runtime_path` / `fallback_to_user_provider` / `notify_on_fallback` / `request_timeout_seconds`）、`/user/info` 余额探针与 `BALANCE_LOW` 分级降级、Key fragments 与发布公钥的 build-time 内嵌、release workflow 均已接入；真实 Rust Runtime 在私有仓 `openbiliclaw-sponsored-runtime`；离线 eval harness `llm/sponsored_pilot_eval.py` + `scripts/sponsored_pilot_eval.py` 可对 mock/official runtime 跑 `soul.consolidation` 输出契约冒烟；已用真实 SiliconFlow 在干净实例跑通 `profile-consolidate --full --apply`：旧流程 prompt + batch 32，免费 `THUDM/GLM-4-9B-0414` 21 个嫌疑簇单批返回 `merges=20 / errors=0 / likes 44→25`，0 fallback，真实 token 写入项目 `llm_usage`，`provider=sponsored` 成本估算固定 ¥0；免费候选实测：R1-0528-Qwen3-8B 空闲时最强但当前服务端对 consolidation payload 返回 400，Qwen3-8B/GLM-Z1-9B/Qwen2.5-7B 会串簇，Qwen3.5-4B 503，Xing4 0 ops，故 pilot policy 固定 GLM-4-9B-0414；多平台打包/签名尚未接入。设计见 [Sponsored Runtime 设计](../plans/2026-09-22-sponsored-runtime-design.md)。 |
