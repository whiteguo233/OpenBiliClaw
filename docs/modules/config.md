...
| `allowed_extension_ids` | list[string] | `[]` | 允许连接的浏览器扩展 ID 白名单。存储 key 派生的扩展 ID（如 `<id>`），认证时从请求 origin（`chrome-extension://<id>` 或 `moz-extension://<id>`）中提取 ID 部分进行匹配。仅当 `verify_extension_id=true` 时生效。**仅 TOML** |
| `verify_extension_id` | bool | `false` | 是否启用扩展 ID 白名单校验。`true` = 只允许 `allowed_extension_ids` 中的扩展连接；`false` = 任意扩展均可连接（适合私有部署）。远程请求生效，本地请求免除（`local = trusted` 原则） |
