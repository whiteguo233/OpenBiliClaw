...
> 从用户画像出发，在 B 站、小红书、抖音、YouTube、X、知乎、Reddit、Bangumi、Linux.do、V2EX、微博、GitHub 和通用 Web 等来源主动寻找潜在会喜欢的内容。
...
- **sources.platforms** — 十二个 canonical 平台族的唯一可枚举注册表；discovery 已看过滤、pool 配额统计、事件身份和 URL host 推断统一复用别名 / strategy 前缀规则。`github/gh` 与 `github.com/<owner>/<repo>` 归入 `github`，但 repository canonical identity 仍以 API 数字 ID 为准
- **SourceAdapter 协议** — 多源适配层（`sources/`），在上述 4 个 B 站策略之外挂载非 B 站内容源（小红书、抖音、YouTube、X、知乎、Reddit、Bangumi、Linux.do、V2EX、微博、GitHub）
...
3. **统一入待评估池**
   虽然四个 B 站策略、小红书被动 / 任务结果、抖音 search / hot / feed、YouTube search / trending / channel、知乎插件任务、Reddit rdt / 插件任务、Linux.do / V2EX 任务、Bangumi 官方 API、微博匿名 client 与 GitHub 官方 REST 的找法不同，但产出都会被转成同一个结构：`DiscoveredContent`，再由 `DiscoveryCandidatePipeline.enqueue_candidates()` 写入 SQLite `discovery_candidates`。
...
4. **混源连续评估**
   ...
   `[discovery].eval_scorer` 控制候选相关性校准模式。默认 `"llm"` 保持既有批量评估和缓存行为，在桌面 Web 与扩展设置的「高级功能」Tab 中显示为 `Agent（默认）`；同一下拉还可显式选择 `Shadow（校准观察）` 与 `Learned（仅相关性，实验性）`，保存后通过 `PUT /api/config` 热重载。注册到顶层 discovery engine 的策略及其回填变体复用同一个 evaluator，因此直连 `discover()` 与统一候选流水线都遵循该全局模式；未注册、独立运行的策略保留默认 Agent evaluator。单条 `evaluate_content()` 在校准模式下也转入同一 batch 校准路径。`"shadow"` 同时运行 `LearnedRelevanceScorer` 与完整 LLM evaluator，但产品相关性仍以 LLM 为准，并把完整、隐私安全的对照对写入审计表；`"learned"` 是人工运行只读 gate 并确认通过后的显式 opt-in，只在完整 LLM 成员已提供 temporal / topic / style / franchise 元数据且本批审计完整落库时，用 learned 分数覆盖 relevance。校准模式会绕过 normal eval cache，并把同时配置的 prefilter `enforce` 当成 `shadow`，避免缓存命中或预过滤制造不完整 cohort；因此当前版本的 `"learned"` 仍会调用 LLM，不能作为节省 LLM 成本的开关。上文“worker 只跑 LLM”只描述默认 `llm` 路径；校准模式的 worker 会在同一个 evaluator 阶段运行 learned + LLM。学习器不可用、异常、分数长度不符、非有限值、向量维度不一致、特征摘要非法或审计写入失败时，产品结果都 fail-open 到 LLM。模式切换仅作用于随后进入评估的候选，不会重算或清除已有推荐池。
...
这是 discovery 里最关键的一类 prompt。runtime 的统一待评估池会把十二个平台来源的候选（含 GitHub repository）交给 `ContentDiscoveryEngine.evaluate_content_batch()`；直接 discover 兼容路径仍可逐条调用 `evaluate_content()`。
...
此前十二个平台的 search 关键词生成器（B 站 `search`、小红书 `xhs-search`、抖音 `search`、YouTube `yt_search`、X `x-search`、知乎 `zhihu-search`、Reddit `reddit-search`、Bangumi `bangumi-search`、Linux.do `linuxdo-search`、V2EX `v2ex-search`、微博 `weibo-search`、GitHub `github-search`）各自独立调 LLM、各发一份画像。统一 planner 把它们收敛成一套「双缓冲 + 缺口拉动」的背压模型，接管各平台 **search 关键词**，并接管 B 站 `ExploreStrategy` 的 `keyword_kind="explore"` query cache 生成；`trending / related / hot / feed / channel / creator / subreddit / ranked / latest` 及各自的 budget/cadence **原样不动**。关键词的 legacy / hybrid / inspiration 生成模式只决定词从哪里产生，不关闭 V2EX 正式 Search 的已配置 Exa / You 召回；外部 provider 不可用时，V2EX 才在 latest/hot 上做精确优先、核心词受限放宽的匿名 fallback。微博 `hot` 只把公开热搜词变成本轮查询种子，不会把热搜条目本身写成候选；GitHub `ranked/latest` 也不消费 planner 关键词。
...
**P2 打磨（供给优势 / 弃权 / 轮换）**：合并 prompt 的静态 system 里有十二来源**平台供给优势表**：B站学习区/梗、小红书生活/美妆、抖音娱乐/热点、YouTube 英文长内容、X 实时/英文、GitHub 公开仓库/开源工程、知乎中文问答/深度回答/经验复盘、Reddit subreddit 经验讨论/技术问答/开源项目、Bangumi ACG 作品名/作者或监督/题材/目录检索、Linux.do 中文技术社区主题/故障排查/开源实践、V2EX 技术与生活讨论/折腾复盘、微博中文公共话题/实时讨论/创作者博文。模型据此把用户兴趣映射到各平台真正有货的方向，并允许**弃权**——某平台供给与用户兴趣不匹配时可少出或返回 `[]`。planner 区分「弃权」与「调用失败」：合并调用**成功**但某平台返回空 = 故意弃权（**不**回退确定性兴趣名、本轮跳过）；**整次调用失败**才对所有 due 平台回退兴趣名。轮换上 `claim_keywords` 严格 FIFO（最旧 pending 先出）；非弃权平台仍低于低水位时，`recycle_oldest_used` 只补充超过近期窗口的历史有效词，近期词优先留给下一轮新生成。
...
| 统一待评估候选池 | ✅ | B 站、XHS、抖音、YouTube、X、知乎、Reddit、Bangumi、Linux.do、V2EX、微博、GitHub 的原始候选先写入 `discovery_candidates(pending_eval)`；API runtime 先用 supply fill loop 按 `pending_eval + evaluating` 补足有效水位，入库前过滤历史候选和已缓存内容，再由唯一 `CandidateEvalCoordinator` 按 `[scheduler].eval_min_batch_size / eval_max_wait_seconds`（默认 15 / 90 秒）凑批，以最多 3×30 连续评估并串行 commit/admission；手动 CLI 固定 `1 / 0` 立即 drain。各路径共享 tokenized claim、统一阈值与 projected-inventory 规则。 |
...
| M123 按平台缺口补池子 | ✅ | runtime 在补货时会先按 `[scheduler.pool_source_shares]` 统计平台族余量；当前十二来源默认保存为 `bilibili=5`、GitHub 等其余 canonical source 均为 `1`，但默认只有 B 站启用，disabled 平台会从有效配比中剔除。B 站缺口会按前端真实可换来源数计算，并用 raw-material headroom 夹住请求量；其余来源分别交给对应 producer。所有来源再统一进入 `discovery_candidates` batch 评估；超 raw-ceiling 配额的平台族才会被压回 raw 配额内 |
...
| 多平台发布时间元数据 | ✅ | 十二个平台来源都只从语义明确的上游字段提取发布时间；GitHub repository 使用 `created_at`，不用发现、Star 或 `updated_at` 冒充发布日期。`published_at` 统一规范为 UTC RFC 3339，只有相对时间时写 `published_label`。两字段贯穿 `DiscoveredContent` → `discovery_candidates` → `content_cache`，重新发现时空值分别保留已有非空值；缺失/异常值不影响候选入队。旧缓存不联网回填，也不以发现时间、任务时间、互动时间或推荐生成时间代替发布时间。 |
...
| Linux.do 扩展同源 discovery | ✅（真实 Chrome E2E） | Linux.do 平台族 `source_platform="linuxdo"`（`linux.do` / `l站` alias）。`LinuxdoDiscoveryProducer` 通过扩展只读执行 search / hot / feed / creator / related，canonical topic identity 为 `topic:<id>`；`_t` 仅布尔、Cookie/原始响应不上传，五分支共享关键词 planner、候选池与 evaluator。2026-08-09 已验证 search/hot/feed/creator 返回 topic、related 合法 empty、正式候选评估与入池 |
| V2EX 官方 API / Feed discovery | ✅ | V2EX 平台族 `source_platform="v2ex"`。`V2EXDiscoveryProducer` 执行 `search / node / tab / hot / latest`，共享关键词 planner、Node/Tab 配置、分支预算和 rate-limit cooldown；PAT 只增强 API 2.0，401/403 自动降级匿名。Topic 归一为 `content_type="topic"` 文字卡，Reply 不独立入池；扩展四个只读 bootstrap scope 已进入 guided init，Reply 按 Topic 聚合，Node affinity 可作为 Node 召回 fallback |
| GitHub 官方 REST repository discovery | 🧪 | GitHub 平台族 `source_platform="github"`（`gh` alias）只接入公开 repository。`GitHubDiscoveryProducer` 执行 `search / ranked / latest` 并共享关键词 planner、待评估池、来源份额和持久化 cursor；`latest` 限定近 30 天创建后按最近更新排序。查询强制 `is:public` 且归一层再次拒绝私有行；匿名实测已覆盖三分支，真实 PAT 与完整 LLM/浏览器验收仍按 GitHub acceptance ledger 保持 BLOCKED / NOT_RUN |
...
- `LearnedRelevanceScorer.score_batch()` 返回 `LearnedBatchResult(scores, available, features_digest)`；在稠密 cosine 之上融合 BM25 词面信号（`bm25_weight` 可调，构造时校验 `0 ≤ weight ≤ 1`），只有完整、有限、维度一致的 embedding 输入才会标记可用。批内无任何词面命中时保持稠密分不变。
- `discovery/bm25.py` 提供 `cjk_tokenize()`（CJK unigram + 字 bigram + 拉丁词分词，无第三方依赖）与内存 `BM25Index`。
- **已知限制**：BM25 的 idf 与批内 max 归一化来自同一批候选，同一候选在不同分批 / cache 命中 / prefilter 组合下分数可能不同；`shadow` 校准 cohort 需要冻结分批方式，绝对分不可跨批次直接比较。
...
### GitHub 公开 repository 来源

GitHub v1 只接入 repository，不把 issue、pull request、commit、release 或用户建模为内容。
`github_repository_to_content()` 使用 `github:repository:<numeric id>` 作 canonical identity，
保留 `node_id`，并把 repository URL、owner/name、description、topics、language、
license、`created_at` 与 stars 映射到统一候选；只有 `stargazers_count`
映射到 `favorite_count`，forks、open issues 和 watchers 不冒充评论、转发或其它互动。

`GitHubDiscoveryProducer` 的三条正式分支为：

- `search`：消费统一 keyword planner 的 canonical `github` 关键词；CLI smoke、正式 producer 与 inspiration backend 共用 public query sanitizer，先移除控制字符和私有范围 qualifier，再由服务端追加 `in:name,description,readme is:public fork:false`。清洗后为空的 planner 词不发请求并正常释放 claim。
- `ranked`：使用 GitHub repository search 按 stars 倒序；这是可回放的公开搜索，
  不命名成或伪装成 GitHub Trending。
- `latest`：以 `created:>=...` 限定近 30 天创建窗口并按最近更新时间排序，同样只保留公开非 fork repository。

无 PAT 时匿名 discovery 就绪；可选 PAT 只提高官方 API 额度并供 `/user`
身份校验。即使 PAT 能看到私有仓库，server-owned query 仍强制
`is:public`，normalizer 仍拒绝 `private=true`。401 会降级为匿名公开发现，
429 按 `Retry-After` 开启来源级持久 cooldown；正式 producer 与 inspiration provider 共用该状态，任一路径进入冷却都会让另一条 fail-closed 跳过，而不是继续撞限额。这条链路完全由后端 client 拥有，
不生成浏览器插件任务、不读 Cookie、不执行 star/watch/follow 或原生保存。
具体配置和验收状态分别见 [GitHub 来源模块](github.md) 与
[验收 ledger](../platform-source-acceptance.github.md)。
...
- 配额单位是“平台族”，不是 raw `content_cache.source`。B 站的 `search` / `related_chain` / `trending` / `explore` 统一计入 `bilibili`；小红书的 `xhs-extension-*` 统一计入 `xiaohongshu`；抖音的 `dy-plugin-*` / `douyin*` 统一计入 `douyin`；知乎的 `zhihu-*` 统一计入 `zhihu`；Reddit 的 `reddit-*` 统一计入 `reddit`；Bangumi 的 `bangumi-*` 统一计入 `bangumi`；Linux.do 的五个 `linuxdo-*` strategy 统一计入 `linuxdo`；V2EX、微博和 GitHub 的各自 strategy 也分别统一计入 `v2ex`、`weibo`、`github`。
- B 站缺口仍由 `ContentDiscoveryEngine.discover()` 的四个策略补齐；小红书缺口由 `XhsTaskProducer` / 浏览器插件任务链补齐；抖音缺口由 runtime `DouyinDiscoveryProducer` 调用 `DouyinDiscoveryService(cache=False, evaluate=False)` 拉 raw candidates，小缺口用 feed / hot 快速补零散名额，大缺口用 search + hot/feed 轮换并统一写入待评估池；YouTube 与 X 由各自后台 producer 补齐；知乎缺口由 runtime `ZhihuDiscoveryProducer` 入队插件 search / hot / feed / creator / related；Reddit 缺口由 runtime `RedditDiscoveryProducer` 默认调用 rdt-cli，命令后端不可用时入队插件 fallback；Bangumi、GitHub 与微博缺口分别由对应后台 producer 通过其只读后端 client 补齐；Linux.do 与 V2EX 缺口由对应 producer 入队只读浏览器任务。所有分支仍只写统一待评估池。X 的 twitter-cli、Reddit 的 rdt-cli / OpenCLI 与 GitHub 后端 client 遵循 `[network].mode`；浏览器 fallback 跟随浏览器网络设置，GitHub 无 fallback。微博固定 `trust_env=false` 国内直连，不受该海外代理配置影响。
...
- `content_type` — 内容形态，复用候选池既有 shape 字段：`"video"`（默认）/ `"note"`（小红书）/ `"tweet"` / `"thread"`（X）/ `"subject"`（Bangumi）/ `"post"`（Linux.do / 微博）/ `"repository"`（GitHub）。GitHub 卡片正文由 description / topics 组成，不伪造媒体字段。`to_cache_kwargs()` 透传正文、形态与目录评分字段，并经 storage migration 补齐旧库列；候选转换优先取 `item.content_type`，保证文字和条目候选不被强标成 `video`
