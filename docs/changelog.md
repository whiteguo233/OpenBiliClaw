...
- **P1/P3 去掉负向惩罚，改为纯正向加成**：原「正向加成 − 负向惩罚」在真实数据上系统性抵消 pos 信号——二元 like/dislike 反馈分不出"视觉品味"与"题材/节奏/UP 风格"的不喜欢，liked/disliked 封面在视觉空间重叠，neg 质心 sim 与 pos 几乎持平（P1 neg p50 0.440 > pos 0.405；P3 neg p50 0.395 ≈ pos 0.397），过半候选的 pos 被 neg 罚分抵消（P1 vp mean 仅 0.008；P3 75% equipped 候选 kf=0）。改为纯向后释放了 pos 信号：P1 vp nonzero 439→1067、mean 0.008→0.024；P3 kf nonzero 25→49、mean 0.012→0.022；方向正确（kigurumi/AI 桌宠/MV 等匹配 liked 品味的内容被抬升）。neg 质心仍构建/持久化，留作未来条件惩罚的快速恢复；根因（反馈语义不分视觉维度）留 C 方案单独处理。
- **跨平台公平性：四路叠加 bonus 按平台 min-max 归一化**：Bilibili-only 信号（P2 弹幕 / P3 关键帧）结构性抬高 Bilibili 候选、挤压 bangumi/xhs——开启 P2 时 bangumi 在 top-25 从 3 掉到 1，纯因 Bilibili 行多了 +0.05 通道而 bangumi 永远追不上。`_normalize_bonus_per_platform` 在 `serve()` 叠加成 `combined_bonus` 后、喂给 MMR 选择器前，按 `source_platform` 分组 min-max 归一化到 `[0, _COMBINED_BONUS_CAP=0.20]`（四路 cap 之和）。缺信号的平台只丢**平台内**区分度，不丢**跨平台**高度：实测 bangumi combined max 0.092→0.200 追平 bilibili，top-25 bangumi 0→3（游戏/动漫/鸣潮回到榜单），xhs 0.083→0.114，信号方向不变。`combined_bonus` 为空时 no-op，排序逐字节一致。
- **修复 P3 关键帧预热的静默永久失败**：`prewarm_pool_keyframes` 原无条件打 `keyframes_fetched_at`，即使 `fetch_keyframes` 返回了真实帧但所有 `embed_image` 返回 `[]`（embed 后端临时故障）也打戳——而 `get_candidates_needing_keyframes` 按 `keyframes_fetched_at IS NULL` 且 `relevance_score DESC` 过滤，导致 prewarm 启动时的短暂 embed 故障会**永久排除最高相关性的候选**出 P3（铁律 2 在 bookkeeping 层的重演）。改为仅当结果确定时打戳：`frames==[]`（视频确无 videoshot 数据）或 `embedded>0`；`frames` 非空但 `embedded==0` 留 NULL 下轮重试。
- **弹幕清洗补多字重复单元压缩**：`collapse_repeats` 原只压单字 run，"求你了×12"、"看我看我看我" 等多字刷屏能漏过。新增 `_UNIT_RUN_RE`（lazy 量词 + 数字守卫）压缩多字重复单元，修复两个先前的 bug（数字误伤 "5000电池"→"50电池"、贪心左双 "求你了×12"→"求你了求你了"）。27 个单测覆盖。
- **`get_feedback_covers` 绕过 pool admission**：`rebuild_visual_profile` 原用 `get_recommendations`（带 `confidence>=min_score` 的 pool admission），低置信反馈行被静默丢弃→零质心→P1 no-op。新增 `get_feedback_covers` 读取每一条反馈封面，确保 liked/disliked 封面都进聚类。
