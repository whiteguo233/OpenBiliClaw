...
LISTEN_PORT = int(os.environ.get("LISTEN_PORT", "2119"))
...
CERT_FILE = os.environ.get("CERT_FILE", os.path.join(CERT_DIR, "srv.crt"))
KEY_FILE = os.environ.get("KEY_FILE", os.path.join(CERT_DIR, "srv.key"))
CRL_FILE = os.environ.get("CRL_FILE", os.path.join(CERT_DIR, "ca.crl"))
CA_FILE = os.environ.get("CA_CERT_FILE", os.path.join(CERT_DIR, "ca.crt"))
...
def _ensure_certs() -> None:
    ...
    """
    srv_crt_path = os.path.join(CERT_DIR, "srv.crt")
    srv_key_path = os.path.join(CERT_DIR, "srv.key")
    ...
    if auto not in ("1", "true", "yes"):
        sys.exit(
            ...
            f"             {srv_key_path}\n\n"
            f"  To fix, either:\n"
            f"    1) Copy your cert files into the volume mounted at {CERT_DIR}.\n"
    ...
    # --- Write all files ---
    _write_file(os.path.join(CERT_DIR, "ca.crt"), ca_cert_bytes, 0o644)
    _write_file(os.path.join(CERT_DIR, "ca.key"), ca_key_bytes, 0o600)
    _write_file(os.path.join(CERT_DIR, "ca.crl"), crl_bytes, 0o644)
...
class ProxyHandler(BaseHTTPRequestHandler):
    ...
    def _handle(self):
        ...
        try:
            conn = HTTPConnection(BACKEND_HOST, BACKEND_PORT, timeout=60)
